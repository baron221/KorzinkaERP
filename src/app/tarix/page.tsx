...
function getLogSummary(log: LogEntry, customersMap: Record<number, string>) {
  ...
  const fmtNum = (n: number) => new Intl.NumberFormat("uz-UZ").format(Math.round(n));
  const fmtDateStr = (dStr?: string) => dStr ? new Date(dStr).toLocaleDateString("uz-UZ") : "";
  ...
  switch (log.entity) {
    case "Sale": {
      ...
      const dateStr = fmtDateStr(s.date);
      return [custName, dateStr ? `Sana: ${dateStr}` : "", itemsStr, amtStr].filter(Boolean).join(" | ");
    }
    case "CustomerReturn": {
      ...
      const amtStr = s.totalAmount != null ? fmtNum(s.totalAmount) + " so'm" : "";
      const dateStr = fmtDateStr(s.date);
      return `Vozvrat: ${[custName, dateStr ? `Sana: ${dateStr}` : "", itemsStr, amtStr].filter(Boolean).join(" | ")}`;
    }
    case "Production": {
      ...
      const itemsStr = s.items?.map((it: any) => `R${it.size}×${it.count}`).join(", ") || "";
      const dateStr = fmtDateStr(s.date);
      return [dateStr ? `Sana: ${dateStr}` : "", raw, baskets, itemsStr].filter(Boolean).join(" | ");
    }
    case "Expense": {
      const cats: Record<string, string> = { ELECTRICITY: "Elektr", WAGES: "Ish haqi", FOOD: "Ovqat", MISC: "Boshqa" };
      ...
      const dateStr = fmtDateStr(s.date);
      return [cat, dateStr ? `Sana: ${dateStr}` : "", amt, s.notes].filter(Boolean).join(" | ");
    }
    case "CustomerPayment": {
      const custName = s.customerId && customersMap ? customersMap[s.customerId] : (s.customer?.name || "");
      const amt = s.amount != null ? fmtNum(s.amount) + " so'm" : "";
      const dateStr = fmtDateStr(s.date);
      return [custName, dateStr ? `Sana: ${dateStr}` : "", amt, s.notes].filter(Boolean).join(" | ");
    }
    case "SupplierPayment": {
      const suppName = s.supplier?.name || "";
      const amt = s.amount != null ? fmtNum(s.amount) + " so'm" : "";
      const dateStr = fmtDateStr(s.date);
      return [suppName, dateStr ? `Sana: ${dateStr}` : "", amt, s.notes].filter(Boolean).join(" | ");
    }
    case "RawMaterial": {
      const suppName = s.supplier?.name || "";
      ...
      const dateStr = fmtDateStr(s.date);
      return [suppName, dateStr ? `Sana: ${dateStr}` : "", weight, amt].filter(Boolean).join(" | ");
    }
    case "Customer": {
      return s.name || "";
    }
